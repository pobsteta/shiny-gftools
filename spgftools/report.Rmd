...
df <- bres[[1]]$Tableau2[,c(1,2,9)]
for (reg in 2:length(unique(params$shape["code"]$code))) {
  # df <- cbind(df, bres[[reg]]$Tableau2[,c(9)])
  df <- df %>%
    right_join(params$res[[reg]]$Tableau2[,c(1,2,9)], by=c(essence="essence", categorie="categorie"))
}
names(df) <- c("essence", "categorie", unique(params$shape["code"]$code))
cat("\\newpage ")
print(xtable(df, caption = "Meilleur tarif par région", comment = FALSE, scalebox = 1, include.rownames = FALSE))
```
